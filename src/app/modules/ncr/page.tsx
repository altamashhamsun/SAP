...
export default function NcrPage() {
  ...
  const branchDepartments = (): DeptWithNcrs[] => {
    ...
    for (const ncr of ncrs.filter((n) => n.branch_id === selectedBranch.id)) {
  ...
  const deptNcrs = (): NcrRecord[] => {
    ...
    return ncrs.filter((n) => n.branch_id === selectedBranch.id && n.department_id === selectedDept.id);
  };
  ...
  const getImages = async (findingId: string): Promise<string[]> => {
  ...
  const resolvedCount = ncrs.filter((n) => n.status === "Resolved").length;
      ...
      <div className="sap-dashboard-content">
        ...
        {view === "branches" && (
          <div>
            <div style={{ display: "flex", gap: "1rem", marginBottom: "1.5rem" }}>
              <div style={{ padding: "1rem 1.5rem", background: "#f0f7ff", borderRadius: "8px", border: "1px solid #cde" }}>
                <div style={{ fontSize: "0.75rem", color: "#666" }}>Total NCRs</div>
                <div style={{ fontSize: "1.5rem", fontWeight: 700, color: "#0070f3" }}>{totalNcrs}</div>
              </div>
              <div style={{ padding: "1rem 1.5rem", background: "#f0fff4", borderRadius: "8px", border: "1px solid #cec" }}>
                <div style={{ fontSize: "0.75rem", color: "#666" }}>Resolved</div>
                <div style={{ fontSize: "1.5rem", fontWeight: 700, color: "#16a34a" }}>{resolvedCount}</div>
              </div>
              <div style={{ padding: "1rem 1.5rem", background: "#fff5f5", borderRadius: "8px", border: "1px solid #fcc" }}>
                <div style={{ fontSize: "0.75rem", color: "#666" }}>Not Resolved</div>
                <div style={{ fontSize: "1.5rem", fontWeight: 700, color: "#dc2626" }}>{unresolvedCount}</div>
              </div>
            </div>
            ...
            <div className="sap-plans-list">
              {branches.map((b) => {
                const count = ncrs.filter((n) => n.branch_id === b.id).length;
        ...
        {view === "departments" && selectedBranch && (
          <div>
            ...
            <div className="sap-plans-list">
              {branchDepartments().map(({ dept, ncrs: deptNcrs }) => {
                const resolved = deptNcrs.filter((n) => n.status === "Resolved").length;
                return (
                  <div key={dept.id} className="sap-plan-card" style={{ cursor: "pointer" }}
                    ...
                    <div className="sap-plan-card-summary">
                      {deptNcrs.length} NCR{deptNcrs.length !== 1 ? "s" : ""}
                      {" · "}
                      <span style={{ color: "#16a34a" }}>{resolved} resolved</span>
                      {" · "}
                      <span style={{ color: "#dc2626" }}>{deptNcrs.length - resolved} open</span>
                    </div>
                  </div>
                );
              })}
        ...
        {view === "ncrs" && selectedBranch && selectedDept && (
          <div>
            ...
            {deptNcrs().length === 0 ? (
              <p className="sap-empty-msg">No NCRs for this department.</p>
            ) : (
              ...
              <tr style={{ background: ncr.status === "Resolved" ? "#f0fff4" : "#fff" }}>
                ...
                <td style={tdStyle}>
                  <select
                    ...
                    style={{ ...selectStyle, background: ncr.status === "Resolved" ? "#dcfce7" : "#fef2f2", borderColor: ncr.status === "Resolved" ? "#16a34a" : "#dc2626" }}
                  >
                      ...
                      <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap" }}>
                        {rowImages[ncr.id].map((img, i) => (
                          <a key={i} href={typeof img === "string" ? img : String(img)} target="_blank" rel="noopener noreferrer">
                            <img src={typeof img === "string" ? img : String(img)} alt={`Evidence ${i + 1}`}
